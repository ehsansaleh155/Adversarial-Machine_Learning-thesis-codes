...
def do_train(model):
    ...
    if args.fake:
        data = FakeData(
            [[batch, args.input_size, args.input_size, 3], [batch]], 1000,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--attack-step-size', help='Adversarial attack step size',
                        type=float, default=0.0)
    parser.add_argument('--eval-attack-iter', help='Adversarial attack iteration at evaluation',
                        type=int, default=10)
    ...
    parser.add_argument('--arch', help='Name of architectures defined in nets.py',
                        default='ResNet')
    parser.add_argument('--res2-bottleneck', help='The number of channels in res2 bottleneck',
                        type=int, default=64)
    parser.add_argument('--group', help='The number of groups for resnext',
                        type=int, default=1)
    parser.add_argument('--activation-name', help='activation function used',
                        default='silu')

    # image size
    parser.add_argument('--input-size', help='Size of input image', type=int, default=224)
    ...
    if args.eval:
        ...
        infs_clean = [ClassificationError('wrong-top1', '{}-top1-error'.format('clean')),
                      ClassificationError('wrong-top5', '{}-top5-error'.format('clean'))]
        infs_adv = [ClassificationError('wrong-top1', 'adv_PGD_{}-top1-error'.format(args.eval_attack_iter)),
                    ClassificationError('wrong-top5', 'adv_PGD_{}-top5-error'.format(args.eval_attack_iter))]
        
        nr_tower = max(get_num_gpu(), 1)
        batch = args.batch // nr_tower

        # dataflow = get_val_dataflow(args.data, args.batch)
        dataset_val_clean = get_val_dataflow(args.data, batch, input_size=args.input_size)
        dataset_val_adv = get_val_dataflow(args.data, batch, input_size=args.input_size)
        tower_func = model.get_inference_func(PGDAttacker(args.eval_attack_iter, args.attack_epsilon, args.attack_step_size))
        config = TrainConfig(
            model=model,
            data=QueueInput(FakeData([[batch, args.input_size, args.input_size, 3], [batch]], 1000, random=False, dtype='uint8')),
            callbacks=[
                DataParallelInferenceRunner(dataset_val_clean, infs_clean, list(range(nr_tower)), tower_name='clean'),
                DataParallelInferenceRunner(dataset_val_adv, infs_adv, list(range(nr_tower)), tower_func=tower_func, tower_name='eval-PGD-{}step'.format(args.eval_attack_iter)),
            ],
