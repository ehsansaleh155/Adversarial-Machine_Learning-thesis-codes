...
flags.DEFINE_bool('sat_preprocessing', False,
                  'Whether to use SmoothAdversarialTraining preprocessing.')
...
def model_fn(features, labels, mode, params):
  ...
  def normalize_features(features, mean_rgb, stddev_rgb):
    ...
    if FLAGS.sat_preprocessing:
      return features * IMAGE_SCALE - 1.0
    else:
  ...
  if FLAGS.use_tpu:
    ...
    eval_metric_ops = (
        eval_metrics[0](eval_metrics[1][0], eval_metrics[1][1]) if eval_metrics else None)
    return tf.estimator.EstimatorSpec(
        mode=mode,
        loss=loss,
        train_op=train_op,
    ...
    if FLAGS.mode == 'train':
      ...
      while current_step < FLAGS.train_steps:
        ...
        next_checkpoint = min(current_step + FLAGS.steps_per_eval,
                              FLAGS.train_steps)
        train_est.train(input_fn=imagenet_train.input_fn, max_steps=next_checkpoint)
